...
Run tests with `go test ./...`; a single package with `go test ./internal/llm/`. Coverage is currently limited to reply parsing (`internal/llm/provider_test.go`).
...
3. `internal/llm/provider.go` — the `Provider` interface (`Generate`, `Describe`) plus `New(cfg)`, and everything both backends share:
   - `buildSystemPrompt` — shell-command rules, the prose escape hatch, machine context (cwd, `osName`, `userlandNote` for the GNU/BSD flag split, `$SHELL`), then few-shot examples. Order matters for the small local models: examples sit last, closest to the reply.
